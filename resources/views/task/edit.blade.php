...
@section('content')
    ...
    <form method="post" action="{{ route(isset($newTaskFlag) ? 'task.store' : 'task.update', $task->id) }}" id="task-edit-form">
        ...
        <div class="mb-6">
            <label for="customer_id" class="block mb-2 text-sm font-medium text-slate-500">Customer</label>
            <select name="customer_id"
                ...
                @foreach($customers as $key => $value)
                    <option value="{{ $key }}" @selected(old('customer_id', $task->customer_id) == $key)>{{ $value }}</option>
                @endforeach
            </select>
            @error('customer_id')
